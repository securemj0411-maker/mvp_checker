...
function Hero({ variant = "a" }: { variant?: HeroVariant }) {
  ...
  const sub =
    variant === "a"
      ? "커리큘럼·가격만 정하면 됩니다. 신청 당일 바로 라이브되는 수강신청 페이지에 진짜 광고를 걸어, 당신을 모르는 사람 수백 명이 ‘수강신청’을 누르는지 잽니다(실제 결제는 없습니다). 방문이 기준 표본을 채우면 그때 판정 — 신청 당일 시작해 보통 2~3일이면 살 사람이 있는지, 녹화하기 전에 압니다."
      : "커리큘럼·가격만 정하면, 신청 당일 바로 라이브되는 수강신청 페이지로 진짜 광고를 돌립니다. 모르는 사람 수백 명의 ‘수강신청’ 반응(실제 결제는 없습니다)을 재고, 방문이 기준 표본을 채우면 만들지 말지(Go/No-Go) 답합니다 — 신청 당일 시작해 보통 2~3일이면.";
...
function TourFakeDoor() {
  return (
    <TourRow
      ...
      body={
        <>
          저희가 당신의 강의를, 이미 열려 있는 것처럼 보이는 수강신청
          페이지로 만듭니다. 수강료까지 그대로 보여줍니다. 방문자가 진짜
          강의라고 믿어야 진짜 반응이 나오기 때문입니다. 방문자가 수강신청
          버튼을 누르면 '오픈예정 · 사전알림' 안내가 나옵니다. 수강생에게
          돈을 받는 일은 없습니다.
        </>
      }
      shot={
...
function TourAds() {
  return (
    <TourRow
      ...
      body={
        <>
          지인의 칭찬은 수요가 아닙니다. 구글 광고로 강의를 전혀 모르는 잠재
          수강생을 데려옵니다. 문구는 2~3가지로 나눠 집행합니다. 반응이 없으면
          강의 주제가 문제인지 문구가 문제인지까지 가려내기 위해서입니다.
        </>
      }
      shot={
        <div className="space-y-3">
          ...
          {[
            {
              head: "하루 15분 홈트 챌린지 | 기구 없이 집에서 8주",
              hot: true,
              ctr: "클릭률 4.2%",
            },
            {
              head: "작심삼일 탈출, 8주 홈트 클래스 | 커리큘럼 보기",
              hot: false,
              ctr: "클릭률 1.9%",
            },
          ].map((ad) => (
            ...
            >
              ...
              <p className="mt-1 text-[13px] leading-relaxed text-text-secondary">
                기구 없이 하루 15분. 8주 ₩89,000, 평생 소장. 커리큘럼을
                확인해 보세요.
              </p>
            </div>
          ))}
...
function TourDashboard() {
  return (
    <TourRow
      ...
      body={
        <>
          광고 계정 화면을 같은 링크로 함께 봅니다. 노출·클릭·수강신청 클릭이
          쌓이는 과정을 숨기는 것 없이 그대로 공개합니다. 클릭은 약한
          신호입니다. 저희는 수강료를 본 뒤의 수강신청 클릭까지 측정합니다.
        </>
      }
      shot={
        <div className="rounded-[18px] border border-border bg-surface p-6 shadow-[0_8px_24px_-8px_rgba(10,23,38,0.08),0_36px_80px_-32px_rgba(16,42,86,0.22)] sm:p-7">
          ...
          <p className="mt-6 rounded-[14px] bg-bg-alt px-5 py-4 text-[13px] leading-relaxed text-text-secondary">
            수요가 있어도 수강료가 안 맞으면 만들수록 손해입니다. 수강생 1명을
...
function Process() {
  const steps = [
    ...
    },
    {
      ...
      d: "표본 찰 때까지",
      h: "광고 집행·측정",
      p: "구글 광고를 집행합니다. 문구 2~3종을 나눠 돌리고, 대시보드를 실시간으로 공유합니다. 광고비를 집중할수록 방문이 빨리 차서 빨라집니다.",
    },
    {
      n: "3",
      d: "표본 충족 시점",
...
function Pricing() {
  const tiers = [
    ...
    },
    {
      ...
      desc: "원하는데 돈이 안 되는 건 아닌지, 수강료와 손익까지 확인합니다. 광고비를 가장 많이 태워 판정이 제일 빠른 플랜입니다.",
      lines: [
        "Quick 전체 포함",
