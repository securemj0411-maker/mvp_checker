...
const BRIEF_SCHEMA = {
  ...
  properties: {
      ...
      items: {
        ...
        properties: {
          headline: {
            type: "string",
            description:
              "약속 한 문장. 결과 중심, 기능 나열 금지, 30자 안팎 (예: 퇴근 후 1시간, 엑셀이 무기가 됩니다)",
          },
          angle: {
            type: "string",
    ...
    target_line: {
      type: "string",
      description:
        "타깃 수강생 한 줄. 나이/직업/상황 중 2개 이상 포함 (예: 엑셀이 막막한 입사 1~3년차 직장인)",
    },
    problem_line: {
      type: "string",
      description: "타깃 수강생이 돈이나 시간을 쓰며 겪는, 이 강의로 풀리는 문제 한 줄",
    },
    price_value: {
      type: "integer",
      description:
        "고객 강의를 수강신청 페이지에 표시할 수강료 숫자 하나 (원 단위). 고객이 답한 가격대 구간 안에서, 고객의 결제 방식에 맞는 자연스러운 숫자. 구독이면 월 요금, 단건이면 1강·패키지 가격, 수수료/광고형이면 대표 수강료. 고객 강의의 수강료이지 비즈필터 검증 서비스 가격이 아니다.",
    },
    price_rationale: {
      type: "string",
      description:
        "그 수강료를 제안하는 근거 한 문장. 고객 강의와 유사 강의 시세 관점으로만. 비즈필터(검증 서비스) 관점의 문장 금지.",
    },
    selling_points: {
      type: "array",
      description:
        "고객 강의를 듣는(결제하는) 이유 정확히 3개 (내부 광고 제작용, 고객에게 노출 안 됨). 지금 배우는 법(무료 유튜브·경쟁 강의)의 약점에서 역산. 비즈필터 검증 서비스의 장점(노코드, 광고 세팅 대행, 단건 비용 등)을 절대 적지 말 것. 오직 고객 강의 자체의 수강 이유.",
      items: { type: "string" },
    },
    name_candidates: {
      type: "array",
      description:
        "고객 강의의 검증용 가칭 2~3개. 한글, 발음 쉬움, 강의 내용이 연상되는 이름. '비즈필터', '검증', '팔릴까' 같은 메타 단어 금지. 기존 유명 강의·브랜드와 겹치지 않게.",
      items: { type: "string" },
    },
    excluded: {
      type: "array",
      description:
        "고객 강의에서 이번 7일 검증에 빼는 커리큘럼/타깃 1~3개 (내부 스코프 기록용, 고객에게 노출 안 됨). 반드시 강의의 범위에 대한 것. '광고 채널 자동 집행', '연동 기능' 같은 비즈필터 검증 작업 관련 용어 절대 금지. 빈 배열 금지.",
      items: { type: "string" },
    },
    intake_questions: {
      type: "array",
      description:
        "전화 상담 없이 이 수강신청 페이지·광고를 '만들기 위해' 아직 비는 정보를 고객에게 묻는 질문 정확히 2~3개. 앞 단계(강의 주제·퀴즈·수강료·대안)에서 이미 받은 건 묻지 말 것. 빌드에 바로 쓰이는 것만: 페이지에 넣을 신뢰 요소(강사 실적·경력·자격·수강 후기), 광고에서 꼭 강조할/절대 쓰면 안 될 표현, 비주얼 톤·느낌, 무료 유튜브·기존 강의 대비 한 줄 차별점(아직 불명확하면), 타깃이 검색할 법한 단어 등. 이 강의에 특정한 것만, 일반론 금지.",
      items: {
        type: "object",
        properties: {
          key: {
            type: "string",
            description: "짧은 한글 라벨. 예: 강사 신뢰, 광고 톤, 차별점, 검색어",
          },
          question: {
            type: "string",
...
async function generateBrief(
  ...
  const facts = [
    `강의 주제 원문: ${answers.idea}`,
    answers.ideaRefined ? `강사가 고른 해석: ${answers.ideaRefined}` : null,
    report?.one_liner ? `설계서 한 문장: ${report.one_liner}` : null,
    report?.target ? `설계서 타깃 수강생: ${report.target}` : null,
  ...
  const response = await client.messages.create({
    ...
    system: [
      ...
      "고객(강사·크리에이터)이 만들려는 '고객의 강의'를 광고로 검증하기 위한 브리프 초안을 작성합니다.",
      "",
      "가장 중요한 원칙 (위반 시 전부 무효):",
      "- 검증 대상은 언제나 '고객의 강의'입니다. 비즈필터(이 검증 서비스) 자체를 검증 대상으로 삼지 마세요.",
      "- 고객 강의가 모호하거나 '검증 서비스' 비슷하게 들려도, 비즈필터의 타깃(강의를 만들려는 사람)/오퍼(녹화 전에 검증)/장점(노코드, 광고 대행, 단건 비용)을 결과에 넣지 마세요. 그건 우리 서비스이지 고객 강의가 아닙니다.",
      ...
      "- 모든 문장 경어체. 줄표(—) 금지. 과장 금지. '국내 최초', '최고', '누구나 월 천만원' 같은 검증 안 된 수식어 금지.",
      "- 용어 규칙(설명문에만): price_rationale 처럼 강사에게 설명하는 문장에서 업계 용어를 쓰면 처음 나올 때 괄호로 쉬운 설명을 병기합니다(영어 약자는 한글 풀이 괄호). 단, 오퍼 헤드라인은 최종 수강생용 광고 문구이므로 짧고 강하게 쓰고 괄호 설명을 넣지 않습니다.",
      "",
      "intake_questions(전문가 사전 점검): 위 초안을 실제 수강신청 페이지·광고로 '전화 없이' 만들 때 당신(설계자)이 아직 모르는, 그래서 고객에게 직접 물어야 정확해지는 것 2~3개를 질문으로 만드세요. 각 질문에 고객이 탭할 그럴듯한 보기 3~4개를 미리 채웁니다.",
      "intake_questions 중복 금지(중요): 앞 단계에서 이미 받은 건 절대 다시 묻지 마세요. 구체적으로 '강사가 고른 해석'에 이미 적힌 타깃·핵심 소구(어필 포인트)·차별점, 그리고 수강료·결제 방식·지금 배우는 법은 다시 묻지 않습니다. 이미 만든 target_line/selling_points/offer 와 같은 주제도 다시 묻지 마세요. 오직 '제작에만 필요한 새 디테일'만 물으세요: 페이지에 넣을 신뢰 요소(강사 실적·경력·자격·수강 후기), 광고에서 꼭 쓰거나 절대 쓰면 안 될 표현·규제, 비주얼 톤·느낌, 타깃이 검색할 단어 등.",
    ].join("\n"),
