...
async function interpret(idea: string): Promise<InterpretResult | null> {
  ...
  const response = await client.messages.create({
    ...
    system: [
      ...
      "2) gaps: 그 페이지와 광고를 만들려면 꼭 알아야 하는데 원문만으론 알 수 없는 것 1~2개를 질문으로 만들고, 각 질문에 그럴듯한 답 3~4개를 미리 채워 줍니다(고객은 여러 개 고르거나 직접 적습니다).",
      "규칙:",
      "- gaps는 이 아이디어에 특정한 것만. 가장 값진 빈칸은 보통: 광고에 내세울 핵심 소구, 가장 먼저 팔 구체적 타깃(역할·상황), 기존 방법 대비 차별점.",
