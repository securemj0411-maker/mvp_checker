...
const INTERPRET_SCHEMA = {
  ...
  properties: {
    summary: {
      type: "string",
      description:
        "강의 주제를 이해한 그대로 한 문장. 평서문, 쉬운 말, 경어체. 자신 있게 단정해서 고객이 '맞다'고 끄덕이게. 업계 용어 금지(부득이하면 괄호로 쉬운 풀이). 원문에 없는 가격·수치 지어내지 않기.",
    },
    gaps: {
      type: "array",
      description:
        "정확히 1~2개만(2개를 넘기지 말 것). 전화 상담 없이 이 강의로 가짜 수강신청 페이지와 광고를 만들려면 꼭 알아야 하는데, 원문만으론 알 수 없는 것. 광고 헤드라인·타깃 수강생·차별점처럼 만들기에 바로 쓰이는 것 위주. 'ChatGPT도 할 일반론'(막연한 '타깃이 누구?', '마케팅이 중요') 금지. 이미 원문에 있는 건 묻지 말 것.",
      items: {
        type: "object",
        properties: {
          key: {
...
async function interpret(idea: string): Promise<InterpretResult | null> {
  ...
  const response = await client.messages.create({
    ...
    system: [
      "당신은 온라인 강의 수요검증 회사 '비즈필터'의 인테이크(intake) 전문가입니다.",
      "전화 상담은 없습니다. 강사·크리에이터가 적은 강의 주제 한 줄만으로, 가짜 수강신청 페이지와 광고를 만들 수 있도록 핵심을 잡아야 합니다.",
      "두 가지를 합니다:",
      "1) summary: 강의 주제를 한 문장으로 이해한 그대로 자신 있게 되비춥니다(확인용).",
      "2) gaps: 그 수강신청 페이지와 광고를 만들려면 꼭 알아야 하는데 원문만으론 알 수 없는 것 1~2개를 질문으로 만들고, 각 질문에 그럴듯한 답 3~4개를 미리 채워 줍니다(고객은 여러 개 고르거나 직접 적습니다).",
      "규칙:",
      "- gaps는 이 강의에 특정한 것만. 가장 값진 빈칸은 보통: 광고에 내세울 핵심 소구(배워서 뭐가 되는지), 가장 먼저 들을 구체적 타깃 수강생(역할·상황), 무료 유튜브·경쟁 강의 대비 차별점.",
      "- 질문은 복수 선택을 허용하는 표현으로. '가장 큰 이유 한 가지' 같이 하나만 강제하지 말고 '어떤 점인가요?'처럼 열어 둘 것(고객이 여러 개 고를 수 있음).",
    ...
    messages: [{ role: "user", content: `강의 주제: ${idea}` }],
  });
...
const REPORT_SCHEMA = {
  ...
  properties: {
    understanding_line: {
      type: "string",
      description:
        "'우리가 이해한 건 이겁니다' 톤의 한 문장. '[타깃 수강생]이 [지금 배우는 법] 대신 이 강의를 듣게 만드는 것' 형태로, 고객이 자기 강의를 거울로 보고 '맞다'고 끄덕이게. 가격 숫자는 넣지 않는다.",
    },
    ...
    top_risk: {
      type: "string",
      description:
        "이 강의에서만 나오는 가장 날카로운 리스크 1개. ChatGPT도 말할 일반론('경쟁이 치열', '마케팅이 중요') 절대 금지. 이 강의 주제/타깃/분야에 특정한 것 하나만(예: 무료 유튜브와의 차별). 전문가가 짚어주는 한 스푼이 되도록.",
    },
    blind_spot: {
      type: "string",
      description:
        "이 강의에서 7일 광고로는 답을 못 내는 구체적 변수 1개. 일반적 겸손('시장만 안다') 금지. 이 강의 종속적으로 (예: '완강률·재수강은 7일로 안 보입니다').",
    },
  },
  required: [
    "understanding_line",
...
const LABELS: Record<string, Record<string, string>> = {
  ...
  build: {
    self: "수강신청 페이지를 직접 만들 수 있음 (바이브코딩, 노코드)",
    need: "수강신청 페이지 제작이 필요함",
    built: "수강신청·소개 페이지가 이미 있음",
  },
  audience: {
    b2c: "개인 수강생",
    b2b: "기업·기관 (B2B 교육)",
  ...
  revenue: {
    once: "한 번 결제(단강·패키지·전자책)",
    subscription: "정기 결제(구독·멤버십·월 회비)",
    usage: "들을 때마다 결제(회차·세션마다)",
    fee: "수수료나 광고",
    undecided: "미정",
  },
  price: {
  ...
  alternative: {
    competitor: "비슷한 유료 강의·학원을 이용 중",
    manual: "유튜브·블로그·책으로 독학",
    none: "안 배우고 그냥 참고 있음",
    unaware: "배울 수 있는 줄도 모름 (필요성을 알려줘야 하는 시장)",
    unknown: "모름",
  },
  region: {
...
async function generateReport(a: QuizAnswers): Promise<Report | null> {
  ...
  const facts = [
    `강의 주제 원문: ${a.idea}`,
    a.ideaRefined ? `강사가 고른 해석: ${a.ideaRefined}` : null,
    `강의 형태: ${LABELS.service[a.service]}`,
    `수강료 내는 사람: ${LABELS.audience[a.audience]}`,
    `수강료 방식: ${LABELS.revenue[a.revenue]}`,
    `수강료대: ${LABELS.price[a.price]}`,
    `지금 배우는 법: ${LABELS.alternative[a.alternative]}`,
    a.region ? `주 수강생 범위: ${LABELS.region[a.region]}` : null,
  ...
  const response = await client.messages.create({
    ...
    system: [
      "당신은 온라인 강의 수요검증 회사 '비즈필터'의 검증 설계자입니다.",
      "강사·크리에이터가 막 강의 주제를 입력했습니다. 당신의 역할은 긴 리포트를 쓰는 게 아니라, (1) 우리가 그 강의를 정확히 이해했음을 거울처럼 보여주고, (2) 전문가만 짚을 수 있는 관점 한 스푼을 주는 것입니다. ChatGPT도 뱉을 일반론은 신뢰를 깎습니다.",
      "",
      "채널과 합격선은 회사가 이미 정했습니다 (반드시 이 값을 쓰되, 이유는 이 강의에 맞게 다시 쓰세요):",
      `- 추천 채널: ${ch.channel}`,
      `- 합격선: ${pb.bar}`,
      "",
      "작성 규칙:",
      "- understanding_line: 고객이 읽고 '맞다'고 끄덕이게. 자기 입력의 단순 반복이 아니라 한 단계 정리된 한 문장. '[타깃 수강생]이 [지금 배우는 법] 대신 이 강의를 듣게 만드는 것' 형태가 좋습니다.",
      "- top_risk: 이 강의 주제/타깃/분야에만 해당하는 가장 날카로운 리스크 1개. '경쟁이 치열', '마케팅 중요' 같은 일반론 절대 금지. 무료 유튜브·기존 강의 대비 차별 같은 강의 특유의 리스크를 짚으세요.",
      "- blind_spot: 이 강의에서 7일 광고로 답 못 내는 구체적 변수 1개(예: 완강률·재수강·후기 입소문).",
      "- 모든 문장 경어체. 줄표(—) 금지. 과장·판정 표현('성공', '유망') 금지.",
      "- 용어 규칙: 흔히 아는 업계 용어는 써도 되지만, 처음 나올 때 괄호로 쉬운 설명을 한 번 병기합니다. 예: 'CAC(수강생 한 명 데려오는 비용)', '리텐션(재수강률)'. 영어 약자는 반드시 한글 풀이를 괄호로. 너무 어려운 전문용어는 아예 쉬운 말로 풉니다.",
      "- 강의 주제가 모호하면 가장 그럴듯한 가정을 채우되 그 가정을 문장에 드러냅니다.",
    ].join("\n"),
