...
export const REFUND_POLICY = [
  ...
  "될지 안 될지 분명한 결론(Go/No-Go)을 못 드리면: 전액 환불 (판정 보장)",
...
export function decideChannel(a: QuizAnswers): {
  ...
  if (a.alternative === "unaware") {
    return {
      channel: "메타(인스타) 피드 광고",
      reason:
        "문제라고 인식 못 하는 고객은 해결책을 검색하지 않아 검색광고는 노출할 사람 수가 안 나옵니다. 피드에서 문제를 먼저 보여주고 반응을 만드는 방식이 맞습니다.",
    };
  }
  if (a.audience === "b2b") {
    return {
      channel: "구글 검색광고 + 자격 확인 문항이 붙은 신청서",
      reason:
        "B2B는 문제를 인지하고 해결책을 검색하는 사람을 잡는 것이 가장 짧은 경로입니다. 간단한 자격 확인 문항으로 진짜 문의만 거릅니다.",
    };
  }
  return {
    channel: "구글 검색광고",
    reason:
      "이미 문제를 인지하고 해결책을 찾는 사람에게 노출하는 것이 전환까지 가장 짧은 경로입니다. 검색량이 부족하면 메타 피드 광고로 전환합니다.",
  };
}
